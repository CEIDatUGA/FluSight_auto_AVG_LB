...
last_date<-as.Date(max(preliminary_NHSN_data$weekendingdate))

# write so we keep the records
write_csv(preliminary_NHSN_data, paste0(
  "hospitalizations_data/Weekly_Hospital_Respiratory_Data__HRD__Metrics_by_Jurisdiction__National_Healthcare_Safety_Network__NHSN___Preliminary__", last_date, ".csv")
  )
...
submission_forecasts<- bind_rows(states_forecasts, us_forecast) %>%
  ...
  mutate(output_type_id = sub("\\.?0+$", "", output_type_id))
...
file_name <- paste0("UGA_CEID-auto_AVG_LB/", 
                    date_string, 
                    "-UGA_CEID-auto_AVG_LB.csv")
# write CSV
